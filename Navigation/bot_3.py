...
udp_host = "192.168.0.133" 		# Host IP
...
def get_db_data():
	if(is_simulation):
		df_bot = pd.read_csv("../Simulation/bot_3_sim.csv")
		row= df_bot.iloc[-1]
	...
	#its not simulation 
	df_bot = pd.read_csv("../OpenCV/bot_3.csv")
	row= df_bot.iloc[-1]
	bot_x           =int(float(row[2]))
	bot_y           =int(float(row[3]))
	bot_front_x     =int(float(row[4]))
...
class PID_controller:
	...
	pid_steering_controller = PID(10, 0, 0, setpoint=0)  #30,0,0
...
def repeat():
	...
	if abs(angle_error) > 60 or in_pivot_mode:
		...
		if abs(angle_error) >= 35:
			pivot_speed = int(np.interp(abs(angle_error),[35,180],[300,500]))
			print ("pivoting")
			if angle_error > 0:
				#pivot anti-cws
	...
	if in_pivot_mode == False:	 		
		...
		#xyz_direction = 1 for forward
			...
			BOT_3.Steer = BOT_3.pid_steering_controller(angle_error)
			BOT_3.Thr = BOT_3.pid_throttle_controller(-eucledian_dist_error)
	
			#get computed values from PID
			out_left_wheel = BOT_3.Output_left_wheel()
			out_right_wheel = BOT_3.Output_right_wheel()
